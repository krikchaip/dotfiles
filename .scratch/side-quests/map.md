...
- Expose `/subagent-done` only in interactive children; never expose it as a model tool. Plain completion returns the latest response. `--wrap-up` runs one hidden tool-disabled final turn, then shows the complete response in a `WRAP UP` banner. Success saves the response and closes the child. Failure keeps the child open and restores its tools.
