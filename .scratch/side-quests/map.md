...
- Expose `/subagent-done` only in interactive children; never expose it as a model tool. Plain completion returns the latest response. `--wrap-up` runs one hidden tool-disabled final turn and adapts its form to the latest continuation, excluding earlier work. Without a continuation, it starts from the launch prompt and treats inherited parent messages as context only. It then shows the complete response in a `WRAP UP` banner. Success saves the response and closes the child. Failure keeps the child open and restores its tools.
