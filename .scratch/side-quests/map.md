...
- Do not expose a model-callable completion tool. Dynamically register child-only `/subagent-done` only for an initially interactive or terminal-taken-over child, and restore it from persisted lifecycle state after reload or reopen. While autonomous, do not register or show it; reject manually entered command text before it reaches the model or promotes lifecycle. The command takes no arguments. While a turn is active it refuses with a wait-or-interrupt warning. While idle it writes the trusted completion marker, preserves any unanswered parent request and resumable session, delivers completion, and shuts down.
...
- [Agent Interface](tickets/01-agent-interface.md) — `Agent` always launches or resumes asynchronously, treats identity, context, and lifecycle parameters as new-launch-only, returns the canonical child session path immediately, and later hands the final assistant response plus that path back to the parent without injecting the transcript.
