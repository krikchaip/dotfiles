...
- Every child Pi pane can accept terminal input. `interactive` controls lifecycle only on new launch: autonomous panes close after `agent_end`, while persistent panes stay open until explicitly finished. After creation, only submitting an accepted non-command prompt from the live child terminal permanently changes an autonomous pane to persistent mode; `Agent.resume`, ordinary continuation, other extension-injected messages, editing, typing, pasting, navigation, and commands do not. Interactive lifecycle never demotes.
- Do not expose a model-callable completion tool. Dynamically register child-only `/subagent-done` only for an initially interactive or terminal-taken-over child, and restore it from persisted lifecycle state after reload or reopen. While autonomous, do not register or show it; reject manually entered command text before it reaches the model or promotes lifecycle. The command accepts only the optional `--wrap-up` flag. While a turn is active it refuses with a wait-or-interrupt warning. Plain idle completion returns the latest settled response. `--wrap-up` runs one final synthesis turn with no active tools while hiding its model-only request and native assistant output; non-empty output is persisted and shown once as one large `WRAP UP` Markdown banner, becomes the completion response, and then exits. Failure, interruption, or textless output keeps the pane open and restores the prior tools. Successful completion preserves any unanswered parent request and resumable session, delivers completion, and shuts down.
