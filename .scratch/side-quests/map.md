...
- Every child Pi pane can accept terminal input. `interactive` controls lifecycle only: autonomous panes close after `agent_end`, while persistent panes stay open until explicitly finished. Explicit `Agent.resume` with `interactive: true` or submitting a non-command prompt from the child terminal permanently changes an autonomous pane to persistent mode; ordinary continuation, other extension-injected messages, editing, typing, pasting, navigation, and commands do not.
- Do not expose a model-callable completion tool. Dynamically register child-only `/subagent-done` only for an initially interactive, explicitly resume-promoted, or terminal-taken-over child, and restore it from persisted lifecycle state after reload or reopen. While autonomous, do not register or show it; reject manually entered command text before it reaches the model or promotes lifecycle. The command takes no arguments. While a turn is active it refuses with a wait-or-interrupt warning. While idle it writes the trusted completion marker, preserves any unanswered parent request and resumable session, delivers completion, and shuts down.
