...
- Register `/subagent-done` only for the current persisted interactive lifecycle. Restore it after reload or reopen. Autonomous input must not reach the model or promote lifecycle. Never register the command in the parent or expose it as a model tool.
- Accept only optional `--wrap-up`. Reject other arguments with `Usage: /subagent-done [--wrap-up]`. Refuse completion during an active turn. Plain completion returns the latest settled response.
- `--wrap-up` runs one tool-disabled synthesis turn. Hide its instruction and partial response. Scope the handoff from the latest `side-quest-continuation`, excluding earlier work. If no continuation exists, scope it from the hidden `side-quest-launch` boundary before `Agent.prompt`; inherited parent messages remain context only. Choose the response form for that scope without a required template or headings. On success, show and persist the complete response in a `WRAP UP` banner, then close the child. On failure, keep the child open and restore its tools. Use `FROM PARENT`'s label color, 240-character collapse limit, and effective expand hint.
