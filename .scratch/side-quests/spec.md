...
- Replace HazAT's child tool dashboard with one compact above-editor identity line: `[agent label] — task label · autonomous|interactive`, plus `· reply pending` when needed. Truncate task label first on narrow terminals. Do not include tool lists, expansion state, or a new shortcut.
...
- Inspect real screenshots at normal and narrow widths for parent passive/navigation widgets, selected rows, effective key hints, child identity, `reply needed` and `reply pending` states, result collapsed/expanded rendering, clipping, wrapping, stale rows, and pixel-level frame defects.
