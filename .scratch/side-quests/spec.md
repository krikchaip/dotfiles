...
39. As a Pi user, I want only an accepted non-command prompt from a live child terminal to promote that session permanently to interactive mode after creation, so that takeover remains a human action.
40. As a Pi user, I want `Agent.resume`, typing, editing, commands, ordinary parent continuations, and other programmatic messages not to promote lifecycle, so that incidental or model-selected input does not change cleanup behavior.
...
42. As a Pi user, I want `/subagent-done` to support immediate completion or one optional tool-disabled `--wrap-up` synthesis turn, refuse during an active turn, and stay open when synthesis fails or returns no text, so that completion is explicit and streaming or invalid work is not misclassified.
...
- Register exactly one public model tool named `Agent` with this request contract: required non-empty `prompt` and `description`; optional `subagent_type`, `resume`, `inherit_context`, and `interactive`; no additional properties. `description` is preferably two to six words and labels the pane and status row. The parameter descriptions identify `subagent_type`, `inherit_context`, and `interactive` as new-launch-only fields.
...
- `resume` is an absolute managed child session path. When it is present, `prompt` is continuation text and `description` is the new task label. Reject the whole call if it contains `subagent_type`, `inherit_context`, or `interactive`; the persisted session already owns its identity, context choice, and lifecycle.
- Add concise parent system-prompt guidance that `subagent_type`, `inherit_context`, and `interactive` apply only to new launches and must be omitted on resume. Explain that `interactive: true` is for multi-round human dialogue such as decision grilling, requirements discovery, prototype feedback, or human-in-the-loop review; independent work stays autonomous. Mention `/subagent-done --wrap-up` as the final tool-disabled handoff turn for interactive work. Do not add these rules to the short `Agent` tool description.
...
- Keep parent system-prompt additions small and place them in Pi's `Guidelines` section: define the parent-agent and sub-agent actors; define the main-quest and side-quest tasks; make side-quest delegation optional; require coherent independently reviewable outcomes and a return contract; require parent-agent review; identify `subagent_type`, `inherit_context`, and `interactive` as new-launch-only fields; and include the full valid non-general-purpose agent catalog. Include each canonical name and its full description after whitespace normalization only. Do not duplicate the catalog or launch-only rule in the short tool description.
...
- `interactive` controls lifecycle only on new launch: the per-call value overrides frontmatter and total omission defaults false. Reject it on every resume, whether `true` or `false`. Every pane remains terminal-interactive. After creation, only an accepted non-command input with Pi source `interactive` can promote an autonomous session permanently. Ordinary programmatic parent continuation, other extension-injected messages, commands, typing without submission, editing, pasting, and navigation do not promote. Interactive sessions never demote.
...
- Store canonical agent identity, display/task labels, CWD, model, thinking, exact tools, skill/system-prompt snapshot, context choice, lifecycle mode, parent lineage, and schema version in the manifest. Resolved identity, capability, context, lifecycle, and prompt-policy fields cannot be changed by resume. Update only the task label on continuation. Persist human terminal takeover as a permanent lifecycle promotion. Definition and parent-setting changes affect new children only.
...
- Use widget activity states `starting`, `active`, `waiting`, and `stalled`. Completion and terminal failure are messages, not persistent rows. An exhausted provider or agent-loop error is terminal for an autonomous child: write `failed`, close it, remove its row, and wake the parent. The same turn-scoped error in an interactive child remains local when its Pi process is healthy: preserve the pane and session, return its row to `waiting`, and do not write a terminal marker or wake the parent. Autonomous stall and recovery wake the parent; interactive stall and recovery remain widget-only.
- A fatal or nonzero process exit is terminal for both lifecycle modes. When a pane disappears, allow one extra poll for a racing trusted terminal sidecar. Trusted completion, autonomous completion, cancellation, or terminal error wins. Otherwise report `closed`, include the available final response and session path, explicitly state when an unanswered parent request remains, remove the row, reflow, and preserve the session and pending mailbox.
...
- Register child-only `/subagent-done` dynamically for the current persisted interactive lifecycle: at startup for an initially interactive child and immediately after an autonomous child accepts a direct non-command prompt and becomes permanently interactive. Restore that registration after reload, adoption, or reopen from persisted lifecycle state, not only the original launch option. Do not register or show it while autonomous. An autonomous input guard rejects `/subagent-done` and `/subagent-done ...` without sending input to the model or promoting lifecycle. The command accepts only optional `--wrap-up`; reject other arguments with `Usage: /subagent-done [--wrap-up]`. While a model or tool turn is active, keep the pane open and tell the user to wait or interrupt first. Plain idle completion atomically writes trusted completion state and delivers the latest settled assistant response. `--wrap-up` disables all child tools and injects one final model-only synthesis instruction. Hide that instruction and its native assistant rendering. Persist a non-empty final response as one custom transcript entry, render it once as one large `WRAP UP` Markdown banner, use it as trusted completion output, and then shut down while retaining the resumable session. If that turn fails, is interrupted, or returns no text, append no final banner, restore the prior tool set, and keep the child open. An unanswered parent request remains persisted and does not block completion. Never register the command in the parent or expose it as a model tool.
...
- E2E-test autonomous completion; initial and human-promoted interactive completion; rejection of `subagent_type`, `inherit_context`, or `interactive` on both active and stopped resumes; `/subagent-done` absence and guarded input while autonomous; immediate dynamic registration after terminal takeover; registration continuity across reload and reopen; argument rejection; active-turn refusal; plain idle success with and without a pending parent request; successful `--wrap-up` tool-disabled request, hidden intermediate rendering, exactly one persisted final Markdown banner after reopen, child exit, and synthesized parent handoff; failed, interrupted, and textless wrap-up staying open with no final banner and restored tools; autonomous provider/agent-loop terminal failure; interactive provider/agent-loop local failure followed by retry; fatal process failure in both lifecycle modes; current-run response extraction; stale-response exclusion; local child interrupt; parent interrupt isolation; trusted cancellation; unmarked closure; stall; recovery; and resume.
