...
- With existing tmux windows after the parent, assert that each created or replacement shared sub-agent window is inserted directly after the parent while existing window IDs remain intact and indexes shift right. Verify creation-only placement, separate managed windows for parent agents sharing one tmux window, tmux-defined concurrent order, and normal placement only after parent-target disappearance.
