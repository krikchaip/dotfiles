...
- `/subagent-done` exists only for interactive lifecycle and survives reload or reopen. It accepts optional `--wrap-up` and refuses active turns. Plain completion returns the latest response. `--wrap-up` hides one tool-disabled final turn and scopes its adaptive handoff from the latest continuation. Without one, it starts at the hidden launch boundary before `Agent.prompt`, not inherited parent context. It then shows the complete response in a `WRAP UP` banner. Success persists the banner and closes the child. Failure keeps the child open and restores its tools.
