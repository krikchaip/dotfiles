...
- `/subagent-done` is absent while autonomous, and guarded manual input cannot reach the model or promote lifecycle. It registers immediately for initially interactive or terminal-promoted children and remains registered after reload or reopen. Arguments other than optional `--wrap-up` show usage; active turns refuse completion. Plain idle completion returns the latest settled response. `--wrap-up` runs one tool-disabled synthesis turn with its model-only request and native assistant rendering hidden. Non-empty output is persisted and rendered once as one large final `WRAP UP` Markdown banner before the child exits. Failure, interruption, or textless output appends no banner, keeps the pane open, and restores its tools. Both successful forms retain the session and any unanswered parent request.
