...
- Replace the model-callable child `subagent_done` tool with a child-only `/subagent-done` command. Every child Pi pane is terminal-interactive; the public `interactive` flag controls whether the pane persists after `agent_end`, not whether it can accept input. Register the command in every child so it remains available after takeover, and never register it in the parent or expose it to the model. When invoked during streaming or another active agent turn, keep the child open and show a warning notification telling the user to wait or interrupt first. When invoked while idle, atomically write the trusted completion marker and shut down the child.
...
- Fix HazAT's destroyed-surface polling gap: when a tracked pane disappears, allow one additional poll for a racing terminal sidecar. A trusted `done`, autonomous-completion, cancellation, or error marker wins; otherwise terminate tracking as `closed`, wake the parent, remove the widget row, reflow remaining panes, and preserve the session for `Agent.resume`. `closed` is the fallback for any unmarked disappearance, including `/quit`, EOF, clean process exit without semantic completion, `tmux kill-pane`, whole-window closure, or an unrecorded crash. An outstanding `ask_parent` request is nonterminal and does not suppress `closed`; preserve its mailbox so a later `Agent.resume` can answer while reopening the session. A captured nonzero process exit or provider/agent-loop error remains `failed`, not `closed`. Expected pane removal during parent shutdown emits no notification; loss of the enclosing tmux session or server normally ends the parent too, leaving no process to notify.
...
- Extend HazAT's view-only widget into a focusable live-child navigator while preserving its restrained blue border, title/count header, elapsed-time prefix, per-call task label, frontmatter `display_name` agent label with filename fallback, right-aligned activity state, truncation, and narrow-terminal behavior. Passive mode adds no bright controls or permanent selection chrome.
...
- Keep HazAT's compact above-editor widget inside every child pane, but replace its available/denied-tool dashboard with one restrained identity and lifecycle line: `[agent label] — task label · autonomous|interactive`, plus `· reply pending` when a request is pending. The corresponding parent row shows `reply needed`; both indicators remain independent of activity state.
- Resolve `agent label` to frontmatter `display_name` when present, otherwise the canonical filename stem. Never render both. Resolve `task label` to the current required `Agent.description`; update it after live continuation or reopened resume.
- Remove the tools list, denied-tools list, expansion state, and `Ctrl+J` shortcut. On narrow terminals, truncate the task label before agent identity or lifecycle/request state.
...
- Keep the pane and Pi process alive while work continues. Do not suppress normal autonomous completion because a request is pending; a child may complete and close before the parent answers, and its unanswered request remains persisted. The parent poller reads the request, injects one custom help message into the parent session, and wakes the parent without reflowing a live pane.
- A new child receives the launch `Agent.prompt` as a normal user-role message after any inherited conversation. `Agent.resume` is the only response surface, and every resume `prompt` is a persisted custom message rather than a normal user-role message. Its `resume` path corresponds to HazAT's `sessionPath`, `prompt` to `message`, `description` to `name`, and `interactive` to the inverse of `autoExit`. For a live idle child, atomically write the matching response and let the child companion inject the custom message immediately and trigger a turn. For a live active child, queue the custom message as `steer` so it arrives after the current tool batch without aborting. For a closed child, reopen its persisted session in a new focus-preserving pane and inject the custom message as the first continuation while resolving the pending request.
- Lifecycle mode, parent-request state, and activity are orthogonal. The parent widget shows `reply needed`; the child widget shows `reply pending`. Only an accepted non-command prompt submitted from the child terminal with Pi input source `interactive` permanently promotes autonomous lifecycle to interactive and may start work while the parent request remains pending; it never cancels that request. Editing, typing, pasting, navigation, extension commands, and extension-injected parent guidance do not promote lifecycle. Parent response never demotes or promotes lifecycle. Clear the pending request only after the matching response is accepted. Thus autonomous or interactive children may both be idle or active while a reply is pending.
