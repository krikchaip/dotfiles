...
31. As a side quest, I want `ask_parent` to finish my current turn cleanly while keeping my pane alive, so that I can wait without an aborted transcript.
32. As a main quest, I want an `ask_parent` request to wake me, so that blocked child work receives attention.
...
39. As a Pi user, I want an accepted non-command prompt from a child terminal to promote that session permanently to interactive mode, so that takeover is explicit.
40. As a Pi user, I want typing, editing, commands, and parent messages not to promote lifecycle, so that incidental input does not change cleanup behavior.
41. As a Pi user, I want `/subagent-done` to close an idle interactive child cleanly, so that completion is explicit and trusted.
42. As a Pi user, I want `/subagent-done` refused during an active turn, so that streaming work is not misclassified as complete.
...
59. As a Pi user, I want each child to show a compact identity and lifecycle line, so that its role, task, and parent-wait state are clear.
...
- `interactive` controls lifecycle only. Per-call value overrides frontmatter; total omission defaults false. Every pane remains terminal-interactive. Only an accepted non-command input with Pi source `interactive` permanently promotes lifecycle. Programmatic parent continuation, commands, typing without submission, editing, pasting, and navigation do not.
...
- Use a private mailbox for `ask_parent`. Allow one outstanding request. Require it to be the sole tool call in its assistant tool batch. Reject sibling calls or a second pending request before writing.
- A successful `ask_parent` writes a correlated request atomically and returns a successful tool result with `terminate: true`. Do not call `ctx.abort()`. Keep the process and pane alive, suppress autonomous exit for that turn, and continue a healthy idle heartbeat.
- `Agent.resume` is the only parent response interface. For a live idle child, write and inject the matching response immediately as a persisted continuation. For a live active child, deliver it as `steer` after the current tool batch without aborting. For a stopped child, reopen the session and deliver it as the first continuation. Clear the request only after matching response acceptance.
- Keep lifecycle mode, parent-request state, and activity independent. An interactive terminal prompt can start child work while a parent request remains pending; it never clears that request.
...
- Keep the parent widget visually close to HazAT's restrained frame: muted/blue border, title and live count, elapsed time, agent label, task label, and right-aligned activity. Use `display_name` or canonical name, never both. Handle narrow widths without broken borders or stale rows.
...
- Replace HazAT's child tool dashboard with one compact above-editor identity line: `[agent label] — task label · autonomous|interactive`, plus `· awaiting parent` when needed. Truncate task label first on narrow terminals. Do not include tool lists, expansion state, or a new shortcut.
- Register child-only `/subagent-done`. While a turn is active, warn and do nothing. While idle, atomically write trusted completion state and shut down. Never register it in the parent or expose it as a model tool.
...
- E2E-test autonomous completion, initial interactive completion, accepted-prompt promotion, non-promoting edits/commands/programmatic messages, `/subagent-done` refusal/success, local child interrupt, parent interrupt isolation, trusted cancellation, failure, unmarked closure, stall, recovery, and resume.
- E2E-test `ask_parent` as a sole call, sibling-call rejection, duplicate-request rejection, idle wait, active response steering, stopped response reopen, pending request during interactive work, matching-ID acceptance, reload survival, and one parent wake.
...
- Inspect real screenshots at normal and narrow widths for parent passive/navigation widgets, selected rows, effective key hints, child identity, awaiting-parent state, result collapsed/expanded rendering, clipping, wrapping, stale rows, and pixel-level frame defects.
