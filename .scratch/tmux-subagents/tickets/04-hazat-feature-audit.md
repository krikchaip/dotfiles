...
- Lifecycle mode, parent-request state, and activity are orthogonal. Manual terminal input may permanently promote autonomous lifecycle to interactive and may start work while the parent request remains pending; it never cancels that request. Parent response never demotes or promotes lifecycle. Clear the pending request only after the matching response is accepted. Thus autonomous or interactive children may both be idle or active while awaiting a parent response.
