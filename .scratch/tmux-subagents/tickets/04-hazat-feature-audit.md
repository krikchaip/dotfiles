...
- Keep HazAT's compact above-editor widget inside every child pane, but replace its available/denied-tool dashboard with one restrained identity and lifecycle line: `[agent label] — task label · autonomous|interactive`, plus `· awaiting parent` when a request is pending.
...
- Permit one outstanding request per child. Require `ask_parent` to be the only tool call in its assistant tool batch; if siblings exist or a request is already pending, fail before writing a new request. A successful call atomically writes the request and returns a successful result with `terminate: true`. Pi then ends the turn normally without Escape, `ctx.abort()`, an aborted transcript entry, or `Operation aborted` UI.
- Keep the pane and Pi process alive. While a request is pending, suppress autonomous auto-exit after that clean `agent_end`; the child becomes idle and keeps writing a healthy heartbeat. The parent poller reads the request, injects one custom help message into the parent session, and wakes the parent without removing the widget row or reflowing panes.
- `Agent.resume` is the only response surface. Its `resume` path corresponds to HazAT's `sessionPath`, `prompt` to `message`, `description` to `name`, and `interactive` to the inverse of `autoExit`. For a live idle child, atomically write the matching response and let the child companion inject it immediately as a persisted custom message that triggers a turn. For a live active child, inject the matching response as `steer` so it arrives after the current tool batch without aborting. For a closed child, reopen its persisted session in a new focus-preserving pane and use `prompt` as the continuation while resolving the pending request.
- Lifecycle mode, parent-request state, and activity are orthogonal. Only an accepted non-command prompt submitted from the child terminal with Pi input source `interactive` permanently promotes autonomous lifecycle to interactive and may start work while the parent request remains pending; it never cancels that request. Editing, typing, pasting, navigation, extension commands, and extension-injected parent guidance do not promote lifecycle. Parent response never demotes or promotes lifecycle. Clear the pending request only after the matching response is accepted. Thus autonomous or interactive children may both be idle or active while awaiting a parent response.
