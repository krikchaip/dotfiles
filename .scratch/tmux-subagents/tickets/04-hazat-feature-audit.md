...
- Fix HazAT's destroyed-surface polling gap: when a tracked pane disappears, allow one additional poll for a racing terminal sidecar. A trusted `done`, autonomous-completion, help, or error marker wins; otherwise terminate tracking as `closed`, wake the parent, remove the widget row, reflow remaining panes, and preserve the session for `Agent.resume`. `closed` is the fallback for any unmarked disappearance, including `/quit`, EOF, clean process exit without semantic completion, `tmux kill-pane`, whole-window closure, or an unrecorded crash. A captured nonzero process exit or provider/agent-loop error remains `failed`, not `closed`. Expected pane removal during parent shutdown emits no notification; loss of the enclosing tmux session or server normally ends the parent too, leaving no process to notify.
